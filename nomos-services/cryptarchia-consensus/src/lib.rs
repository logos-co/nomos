    ...
    ApiAdapter: nomos_da_sampling::api::ApiAdapter + Send + Sync,
{
    ...
    async fn process_block(
        ...
        blob_validation_strategy: BlobValidationStrategy,
        relays: &CryptarchiaConsensusRelays<
            BlendAdapter,
            BS,
            ClPool,
        ...
        match blob_validation_strategy.validate(&block, relays).await {
            Ok(Some(valid)) => {
                if !valid {
                    tracing::debug!("invalid blobs in the block {:?}", block);
                    return Err((
                        Error::BlobVerificationFailure(block.header().id()),
                        cryptarchia,
                    ));
                }
            }
            Ok(None) => {} // blob validation has been skipped
            Err(e) => return Err((e, cryptarchia)),
        }
...
impl BlobValidationStrategy {
    /// Returns Some(true) if all blobs in the block are valid,
    /// Some(false) if one of them are invalid.
    /// Returns None if the validation was skipped.
    #[expect(
        clippy::type_complexity,
        reason = "CryptarchiaConsensusRelays amount of generics."
    )]
        ...
        DaVerifierBackend::Settings: Clone,
    {
        match self {
            Self::Skip => Ok(None),
            Self::ValidateSampled {
                da_availability_window,
                current_slot,
            } => {
                // Skip validation if the block is out of the availability window
                if block.header().slot() + (*da_availability_window).into() >= *current_slot {
                    return Ok(None);
                }

                // TODO: Trigger sampling and wait until complete.
                ...
                let sampled_blobs = match get_sampled_blobs(relays.sampling_relay().clone()).await {
                    ...
                    Err(error) => {
                        ...
                        return Err(Error::BlobSamplingError(block.header().id()));
                    }
                };
                Ok(Some(
                    block
                        .blobs()
                        .all(|blob| sampled_blobs.contains(&blob.blob_id())),
                ))
            }
        }
    }
