    ...
    RuntimeServiceId: Sync,
{
    ...
    drop((
        ...
        inbound_relay,
        // Don't receive secret PoL info updates, as new messages are not generated for the old session.
        secret_pol_info_stream,
    ));
    loop {
        tokio::select! {
    ...
    Backend: BlendBackend<NodeId, BlakeRng, ProofsVerifier, RuntimeServiceId>,
{
    match event {
        SessionEvent::NewSession(CoreSessionInfo {
            ...
            debug!(target: LOG_TARGET, "Handling NewSession");
        ...
        SessionEvent::TransitionPeriodExpired => {
            debug!(target: LOG_TARGET, "Handling TransitionPeriodExpired");
...
fn compute_and_submit_activity_proof_for_previous_session(
    ...
    {
        info!(target: LOG_TARGET, "Activity proof generated for the previous session: {activity_proof:?}");
        submit_activity_proof(activity_proof);
    } else {
        debug!(target: LOG_TARGET, "No activity proof generated for the previous session");
    }
}
