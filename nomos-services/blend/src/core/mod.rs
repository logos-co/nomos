    ...
    ProofsVerifier: ProofsVerifierTrait,
{
    match event {
        ...
        SessionEvent::TransitionPeriodExpired => {
            compute_and_submit_activity_proof(blending_token_collector);
...
fn handle_finish_session_transition_message(
    ...
    if reply_sender.send(()).is_err() {
        error!(target: LOG_TARGET, "Failed to send reply for FinishSessionTransition message");
    }
}

fn compute_and_submit_activity_proof(blending_token_collector: &mut BlendingTokenCollector) {
