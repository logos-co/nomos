    ...
    Rng: RngCore + 'static,
{
    ...
    fn handle_swarm_event(event: SwarmEvent<EventToSwarm>) {
        match event {
            ...
            SwarmEvent::Behaviour(EventToSwarm::SendError {
                reason,
                peer_id,
                connection_id,
            }) => {
                tracing::error!(target: LOG_TARGET, "Failed to send a message to peer:{peer_id:?} via connection:{connection_id:?}: {reason:?}");
            }
            event => {
