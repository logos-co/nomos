...
//         service_components::{
//             MessageComponents, NetworkBackendOfService, ServiceComponents as
...
// {
//     service_resources_handle: OpaqueServiceResourcesHandle<Self,
// RuntimeServiceId>,     _phantom: PhantomData<(CoreService, EdgeService)>,
// }
...
// ServiceCore<RuntimeServiceId>     for BlendService<CoreService, EdgeService,
// RuntimeServiceId> where
//     CoreService: ServiceData<Message: MessageComponents<Payload:
// Into<Vec<u8>>> + Send + Sync + 'static>
//         + CoreServiceComponents< RuntimeServiceId, NetworkAdapter:
//           NetworkAdapterTrait< RuntimeServiceId, BroadcastSettings =
//           BroadcastSettings<CoreService>,
//             > + Send
//                                 + Sync
//                                 + 'static,
...
//         // We tie the core and edge proofs generator to be the same type, to
// avoid mistakes in the         // node configuration where the two services
// use different verification logic
//         + edge::ServiceComponents<ProofsGenerator =
//           CoreService::ProofsGenerator>
//         + Send
//         + 'static,
//     EdgeService::MembershipAdapter:
//         membership::Adapter<NodeId = CoreService::NodeId, Error: Send + Sync
// + 'static> + Send,
//   membership::ServiceMessage<EdgeService::MembershipAdapter>: Send + Sync +
// 'static,     RuntimeServiceId: AsServiceId<Self>
//         + AsServiceId<CoreService>
//         + AsServiceId<EdgeService>
//         + AsServiceId<MembershipService<EdgeService>>
//         + AsServiceId< NetworkService< NetworkBackendOfService<CoreService,
...
//     fn init(
//         service_resources_handle: OpaqueServiceResourcesHandle<Self,
// RuntimeServiceId>,         _initial_state: Self::State,
...
//         let minimal_network_size = settings.minimal_network_size.get() as
// usize;
...
//         let membership_stream = <MembershipAdapter<EdgeService> as
// membership::Adapter>::new(             overwatch_handle
...
//         let (membership, mut session_stream) =
// UninitializedSessionEventStream::new(             membership_stream,
...
//         let mut instance = Instance::<CoreService, EdgeService,
// RuntimeServiceId>::new(             Mode::choose(&membership,
// minimal_network_size),             overwatch_handle,
//         )
//         .await?;
...
//     <<CoreService as ServiceData>::Message as
// MessageComponents>::BroadcastSettings;

// type MembershipAdapter<EdgeService> = <EdgeService as
// edge::ServiceComponents>::MembershipAdapter;
