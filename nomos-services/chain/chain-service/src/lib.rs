...
use core::fmt::Debug;
use std::{collections::HashSet, fmt::Display, hash::Hash, path::PathBuf, time::Duration};
    ...
    TimeBackend::Settings: Clone + Send + Sync,
{
    ...
    async fn process_block_and_update_state(
        ...
        let (cryptarchia, pruned_blocks) =
            Self::process_block(cryptarchia, block, blob_validation, relays).await?;
    ...
    async fn process_block(
        ...
        //       Until then, mempool service will not be ready: https://github.com/logos-co/nomos/issues/1656
        mark_in_block(
    ...
    async fn initialize_cryptarchia(
        ...
        for block in blocks {
            match Self::process_block(cryptarchia.clone(), block, &SkipBlobValidation, relays).await
            {
