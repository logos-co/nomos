...
use tests::{
    adjust_timeout,
    common::da::{disseminate_with_metadata, wait_for_indexed_blob, APP_ID, DA_TESTS_TIMEOUT},
...
async fn test_get_share_data() {
    ...
    let commitments = client
        .get_storage_commitments::<DaShare>(exec_url.clone(), share.blob_id().try_into().unwrap())
        .await
        .unwrap();
