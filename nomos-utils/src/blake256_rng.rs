...
type Hasher = Blake2b256;

#[derive(Clone)]
pub struct Blake256RngSeed([u8; OUTPUT_SIZE]);

impl Default for Blake256RngSeed {
    fn default() -> Self {
        Self([0; OUTPUT_SIZE])
    }
}

impl From<[u8; OUTPUT_SIZE]> for Blake256RngSeed {
    fn from(seed: [u8; OUTPUT_SIZE]) -> Self {
        Self(seed)
    }
}

impl AsRef<[u8]> for Blake256RngSeed {
    fn as_ref(&self) -> &[u8] {
        &self.0
    }
}

impl AsMut<[u8]> for Blake256RngSeed {
    fn as_mut(&mut self) -> &mut [u8] {
        &mut self.0
    }
}

impl IntoIterator for Blake256RngSeed {
    type Item = u8;
...
// the actual type.
type InnerIterator =
    FlatMap<Enumerate<Repeat<Blake256RngSeed>>, Vec<u8>, fn((usize, Blake256RngSeed)) -> Vec<u8>>;

#[derive(Clone)]
pub struct Blake256Rng(InnerIterator);

impl Blake256Rng {
    fn rehash((i, seed): (usize, Blake256RngSeed)) -> Vec<u8> {
    ...
    fn with_seed(seed: Blake256RngSeed) -> Self {
        let iter = std::iter::repeat(seed)
            .enumerate()
            .flat_map(Self::rehash as fn((usize, Blake256RngSeed)) -> Vec<u8>);
        Self(iter)
    }
...
impl SeedableRng for Blake256Rng {
    type Seed = Blake256RngSeed;
...
impl CryptoRng for Blake256Rng {}

impl BlockSizeUser for Blake256Rng {
    type BlockSize = <Blake2b256 as BlockSizeUser>::BlockSize;
}

impl StreamCipher for Blake256Rng {
...
mod tests {
    ...
    fn test_nistrs_blake() {
        println!("======================BLAKE256 RNG==========================");
        nistrs_rng::<Blake256Rng>(Blake256RngSeed(test_seed()));
    }
