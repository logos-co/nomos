...
use crate::core::with_core::behaviour::{
    tests::utils::{largest_peer_id, smallest_peer_id, SwarmExt as _, TestSwarm},
    Behaviour, Event,
};

#[test(tokio::test)]
async fn dialing_peer_not_supporting_blend_protocol() {
    let mut blend_swarm = TestSwarm::new(|id| Behaviour::with_identity(&id));
...
async fn incoming_attempt_with_max_negotiated_peering_degree() {
    let mut listening_swarm = TestSwarm::new(|id| Behaviour::with_identity(&id));
    let mut dialer_swarm_1 = TestSwarm::new(|id| Behaviour::with_identity(&id));
    let mut dialer_swarm_2 = TestSwarm::new(|id| Behaviour::with_identity(&id));
...
async fn outgoing_attempt_with_max_negotiated_peering_degree() {
    ...
    let mut listening_swarm_1 = TestSwarm::new(|id| Behaviour::with_identity(&id));
    let mut listening_swarm_2 = TestSwarm::new(|id| Behaviour::with_identity(&id));
...
async fn outgoing_attempt_with_duplicate_connection() {
    let mut dialer_swarm = TestSwarm::new(|id| Behaviour::with_identity(&id));
    let mut listening_swarm = TestSwarm::new(|id| Behaviour::with_identity(&id));
...
async fn concurrent_reverse_connections_between_peers() {
    let mut swarm_1 = TestSwarm::new(|id| Behaviour::with_identity(&id));
    let mut swarm_2 = TestSwarm::new(|id| Behaviour::with_identity(&id));
...
async fn replace_existing_with_new_connection() {
    let mut smaller_swarm = TestSwarm::new(|_| {
        Behaviour::with_local_peer_id_and_peering_degree(smallest_peer_id(), 1..=2)
    });
    let mut larger_swarm = TestSwarm::new(|_| {
        Behaviour::with_local_peer_id_and_peering_degree(largest_peer_id(), 1..=2)
    });
