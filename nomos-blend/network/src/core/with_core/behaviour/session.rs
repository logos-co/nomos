...
impl Sessions {
    ...
    pub fn transition(&mut self, new_membership: Membership<PeerId>) {
    ...
    pub const fn clear_previous_session(&mut self) -> Option<Session> {
        self.previous.take()
    }
    ...
    pub fn session_mut(
        ...
        if self.current.negotiated_peer(negotiated_peer).is_some() {
            return Some(&mut self.current);
        }

        if let Some(previous) = &mut self.previous {
            if previous.negotiated_peer(negotiated_peer).is_some() {
    ...
    pub fn is_negotiated(&self, peer_id: PeerId) -> bool {
        self.current.negotiated_peer_with_id(&peer_id).is_some()
            || self
                .previous
                .as_ref()
                .and_then(|s| s.negotiated_peer_with_id(&peer_id))
                .is_some()
    }
}
...
impl Session {
    ...
    pub fn negotiated_peer_with_id(
        &self,
        peer_id: &PeerId,
    ...
    /// Registers a message identifier as exchanged with a peer.
    /// Returns `false` if the identifier was previously registered,
    pub fn register_exchanged_message_identifier(
        &mut self,
