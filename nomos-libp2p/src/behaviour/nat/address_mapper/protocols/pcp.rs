...
impl PcpProtocol {
    pub(crate) async fn map_address(
        ...
        let client = PcpClient::new(local_ip, config)
            .map_err(|e| AddressMapperError::GatewayDiscoveryFailed(e.to_string()))?;

        let is_available = timeout(settings.timeout, client.probe_available())
            .await
            .map_err(|_| {
        ...
        if !is_available {
            return Err(AddressMapperError::PortMappingFailed(
                "PCP server unavailable".to_owned(),
            ));
        }
        ...
        let mapping = timeout(
            settings.timeout,
            client.request_mapping(protocol, internal_port, None),
        )
        .await
        .map_err(|_| {
            AddressMapperError::PortMappingFailed(
        ...
        let external_addr =
            build_external_multiaddr(mapping.external_address, mapping.external_port, protocol);
