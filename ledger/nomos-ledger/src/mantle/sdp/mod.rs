...
use crate::{EpochState, UtxoTree, mantle::sdp::rewards::BlendRewardsParameters};
...
enum Service {
    ...
    BlendNetwork(ServiceState<BlendRewards<RealProofsVerifier>>),
}
    ...
    fn try_apply_header(
        ...
        match self {
            Self::DataAvailability(state) => {
                Self::DataAvailability(state.try_apply_header(block_number, epoch_state, config))
            }
            Self::BlendNetwork(state) => {
                Self::BlendNetwork(state.try_apply_header(block_number, epoch_state, config))
            }
        }
    }
...
impl SdpLedger {
    ...
    pub fn with_blend_service(
        mut self,
        rewards_settings: BlendRewardsParameters,
        epoch_state: &EpochState,
    ) -> Self {
        let service = Service::BlendNetwork(Self::new_service_state(BlendRewards::new(
            rewards_settings,
            epoch_state,
        )));
...
mod tests {
    ...
    fn test_update_active_provider() {
        ...
        let sdp_ledger = SdpLedger::new().with_blend_service(
            config.service_rewards_params.blend.clone(),
            &dummy_epoch_state(),
        );
        ...
        for _ in 0..10 {
            sdp_ledger = sdp_ledger
                .try_apply_header(&config, &dummy_epoch_state())
                .unwrap();
        }
    ...
    fn test_no_promotion() {
        ...
        // Initialize ledger with service config
        let mut sdp_ledger = SdpLedger::new().with_blend_service(
            config.service_rewards_params.blend.clone(),
            &dummy_epoch_state(),
        );
