...
use nomos_core::mantle::{
    ops::leader_claim::{LeaderClaimOp, RewardsRoot, VoucherNullifier, VoucherCm},
    Value,
};
...
impl LeaderState {
    ...
    pub fn try_apply_header(self, epoch: Epoch, voucher_cm: VoucherCm) -> Result<Self, Error> {
        Ok(self.update_epoch_state(epoch)?
            .add_voucher(voucher_cm))
    }
    ...
    /// Any cryptographic proof of correct derivation of the voucher nullifier and
    /// membership proof in the merkle tree is expected to happen outside of this function.
