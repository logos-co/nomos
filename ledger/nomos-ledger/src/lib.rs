...
use nomos_core::{
    mantle::{gas::GasConstants, AuthenticatedMantleTx, NoteId, Utxo, ops::leader_claim::VoucherCm},
    proofs::leader_proof,
};
use thiserror::Error;
    ...
    Id: Eq + Hash + Copy,
{
        ...
        Constants: GasConstants,
    {
        ...
        let new_state =
            parent_state
                .clone()
                .try_update::<_, _, Constants>(slot, proof, voucher, txs, &self.config)?;
...
impl LedgerState {
        ...
        LeaderProof: leader_proof::LeaderProof,
    {
        ...
        let mantle_ledger = self.mantle_ledger
...
mod tests {
    ...
    use nomos_core::{
        mantle::{
            gas::MainnetGasConstants, ledger::Tx as LedgerTx, GasCost, MantleTx, Note,
    ...
    fn test_ledger_try_update_with_transaction() {
        ...
        let mut output_note = Note::new(1, [1; 32].into());
        let pk = [0; 32];
        // determine fees
