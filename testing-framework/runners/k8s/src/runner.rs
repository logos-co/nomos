...
use testing_framework_core::{
    nodes::ApiClient,
    scenario::{
        CleanupGuard, Deployer, Metrics, MetricsError, NodeClients, RunContext, Runner, Scenario,
        http_probe::NodeRole,
    },
...
impl Deployer for K8sRunner {
    ...
    async fn deploy(&self, scenario: &Scenario) -> Result<Runner, Self::Error> {
        ...
        let (node_clients, telemetry, cleanup_guard) = {
            let mut cluster =
                setup_cluster(&client, &port_specs, &descriptors, self.readiness_checks).await?;

            info!("building node clients");
            let node_clients = match build_node_clients(&cluster) {
                Ok(clients) => clients,
                Err(err) => {
                    cluster.fail("failed to construct node api clients").await;
                    drop(cluster);
                    return Err(err.into());
                }
            };

            let telemetry = match metrics_handle_from_port(cluster.prometheus_port) {
                Ok(handle) => handle,
                Err(err) => {
                    cluster
                        .fail("failed to configure prometheus metrics handle")
                        .await;
                    drop(cluster);
                    return Err(err.into());
                }
            };
            let cleanup_guard: Box<dyn CleanupGuard> = Box::new(cluster.into_cleanup());
            (node_clients, telemetry, cleanup_guard)
        };
