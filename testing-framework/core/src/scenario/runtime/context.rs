...
impl RunContext {
    ...
    pub fn new(
        ...
        let run_duration = enforce_min_duration(&descriptors, run_duration);
...
fn calculate_expected_blocks(
    ...
    let Some(slot_secs) = positive(slot_duration.as_secs_f64()) else {
        return 0;
    };
    let Some(run_secs) = positive(run_duration.as_secs_f64()) else {
        return 0;
    };
    let Some(coeff) = positive(active_slot_coeff) else {
        return 0;
    };
    let Some(expected) = positive(run_secs / slot_secs * coeff) else {
        return 0;
    };
